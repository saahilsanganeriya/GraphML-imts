...
from models.kafnet.model import KAFNet
...
def prepare_batch_for_kafnet(batch, device, n_variables=36):
    """
    Convert batch to KAFNet format.
    ...
    Returns:
        Dictionary with batched inputs or None if empty
    """
    # Prepare lists
    all_times = []
    all_vars = []
...
def train_epoch(model, dataloader, optimizer, device, n_variables=36):
    ...
    for batch in pbar:
        try:
            batch_data = prepare_batch_for_kafnet(batch, device, n_variables)
...
def main():
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        if val_rmse < best_val_loss:
            ...
            torch.save(checkpoint, 'results/kafnet_best.pt')
    ...
    # Save final results
    with open('results/kafnet_results.json', 'w') as f:
    ...
    print(f"Results saved to: results/kafnet_results.json")
    print(f"Best model saved to: results/kafnet_best.pt")
