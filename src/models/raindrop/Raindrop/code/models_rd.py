...
import os
# Windows-specific CUDA paths - commented out for Mac/Linux compatibility
...
class Raindrop(nn.Module):
    ...
    def __init__(self, d_inp=36, d_model=64, nhead=4, nhid=128, nlayers=2, dropout=0.3, max_len=215, d_static=9,
                 MAX=100, perc=0.5, aggreg='mean', n_classes=2, global_structure=None, n_forecast_steps=None, forecasting_mode=False):
        ...
        self.edge_type_train = torch.ones([36*36*2], dtype= torch.int64)
        self.adj = torch.ones([36, 36])
        ...
        # MODIFIED FOR FORECASTING
        self.forecasting_mode = forecasting_mode
        if forecasting_mode and n_forecast_steps is not None:
            self.n_forecast_steps = n_forecast_steps
            self.mlp_static = nn.Sequential(
                nn.Linear(d_final, d_final),
                nn.ReLU(),
                nn.Linear(d_final, d_inp * n_forecast_steps),  # Forecasting
            )
        else:
    ...
    def forward(self, src, static, times, lengths):
        ...
        if step2 == False:
            ...
            adj = self.global_structure.to(src.device)
            adj[torch.eye(36).byte()] = 1

            edge_index = torch.nonzero(adj).T
            edge_weights = adj[edge_index[0], edge_index[1]]

            output = torch.zeros([x.shape[0], src.shape[1], 36*self.dim], device=x.device)
            alpha_all = torch.zeros([edge_index.shape[1],  src.shape[1]], device=x.device)
        ...
        # MODIFIED FOR FORECASTING: Reshape output
        if self.forecasting_mode:
            batch_size = output.size(0)
            output = output.view(batch_size, self.d_inp, self.n_forecast_steps)  # (B, D, n_forecast_steps)
...
class Raindrop_v2(nn.Module):
    ...
    def forward(self, src, static, times, lengths):
        ...
        if step1 == False:
            ...
            n_step = src.shape[0]
            output = torch.zeros([n_step, batch_size, self.d_inp*self.d_ob], device=x.device)

            use_beta = False
            if use_beta == True:
                alpha_all = torch.zeros([int(edge_index.shape[1]/2), batch_size], device=x.device)
            else:
                alpha_all = torch.zeros([edge_index.shape[1],  batch_size], device=x.device)
        ...
        if masked_agg == True:
            ...
            mask2 = mask.permute(1, 0).unsqueeze(2).long()
            if sensor_wise_mask:
                output = torch.zeros([batch_size,self.d_inp, self.d_ob+16], device=src.device)
